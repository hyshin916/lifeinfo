...
public class DBSave {
	...
	public String strParse(String parseData) throws IOException, InterruptedException {
		...
		for (int i = 0; i < str1.length; i++) {
			...
			info.setOptionName(str3[7]);
			if (str3.length == 8) {
				info.setBodyImgURL("");
			} else {
				info.setBodyImgURL(str3[8]);	
			}
			info.setIncreOptionValue(str3[9]);			
			dbConnection(info);
...
/*		OutputStreamWriter out = new OutputStreamWriter(uc.getOutputStream());
	...
	public void dbConnection(CrawlInfoVO info) {
		...
		String query = "insert into crawlinfo_tbl(category,viewurl,thumburl,title,price,proprice,size,color,bodyimgurl,thumbfileName,made,reg_date,optionName,increOptionValue,no,userid) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,(select ifnull(max(no)+1,1) from crawlinfo_tbl temp where userid='admin'),'admin')";
		...
		try {
			...
			psmt.setString(13,info.getOptionName());
			psmt.setString(14,info.getIncreOptionValue());
